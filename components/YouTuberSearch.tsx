...
import { Search, Loader2, Eye, Calendar, Sparkles, ChevronDown } from 'lucide-react';
import { searchChannelVideos } from '../services/youtubeService';
import { SearchedVideo, YouTubeSortOption } from '../types';
...
export const YouTuberSearch: React.FC<YouTuberSearchProps> = ({ onSelectVideo }) => {
  ...
  const [sortOrder, setSortOrder] = useState<YouTubeSortOption>('relevance');
  ...
  const handleSearch = async (e?: React.FormEvent, isLoadMore = false) => {
    ...
    try {
      const response = await searchChannelVideos(
        query,
        isLoadMore ? nextPageToken : undefined,
        sortOrder
      );

      if (response.videos.length === 0 && !isLoadMore) {
        setError("No videos found.");
      } else {
        setResults(prev => isLoadMore ? [...prev, ...response.videos] : response.videos);
        setNextPageToken(response.nextPageToken);
      }
    } catch (err: any) {
      if (err.message.includes("403")) {
        setError("YouTube Data API not enabled. Check console.");
      } else {
  ...
  const handleSortChange = (newSort: YouTubeSortOption) => {
    setSortOrder(newSort);
    if (query.trim()) {
      setResults([]);
      setNextPageToken(undefined);
      // Trigger new search with new sort order
      setTimeout(() => handleSearch(undefined, false), 0);
    }
  };
  ...
  return (
    <div className="space-y-3 animate-in fade-in duration-300">
      ...
      <div className="relative">
        <select
          value={sortOrder}
          onChange={(e) => handleSortChange(e.target.value as YouTubeSortOption)}
          className="w-full px-3 py-2 pr-8 text-xs bg-dark-900/50 border border-gray-700 rounded-lg text-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 appearance-none cursor-pointer"
        >
        ...
        <div className="space-y-2">
          ...
          {nextPageToken && (
            <button
              ...
              className="w-full py-2 px-4 bg-gray-800/80 hover:bg-gray-700 text-white rounded-lg text-xs font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loadingMore ? (
                <>
